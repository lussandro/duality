  ...
  const verifyToken = async (token: string) => {
    try {
      ...
      if (data.data) {
        ...
        addToast('Sessão inválida.', 'error');
      }
    } catch (error) {
      console.error(error);
      document.cookie = 'token=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;';
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (error && error.response && error.response.data) {
        if (error.response.data.message === 'Seu plano está expirado.') {
          router.push('/ativar');
        } else {
          addToast(error.response.data.message, 'error');
        }
      } else {
        addToast('Erro ao processar o login.', 'error');
      }
    }
  };
      ...
      <div className="rounded px-8 py-8 md:w-2/3 w-full max-w-lg mx-auto">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-center">
            <button
              type="submit"
              className="bg-blue-500 hover:bg-blue-700 transition-all duration-300 font-medium py-2 px-4 rounded outline-none"
            >
              Entrar
            </button>
          </div>
